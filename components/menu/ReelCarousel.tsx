...
export default function ReelCarousel({ items }: ReelCarouselProps) {
  ...
  return (
    <div
      className="relative flex min-h-[560px] w-full items-center justify-center overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-[#0b0f1c] via-[#101425] to-[#070911] px-6 py-10 text-white md:min-h-[640px]"
      ...
      <div className="flex w-full max-w-4xl flex-col items-center gap-6 md:flex-row md:gap-10">
        <div className="relative w-full max-w-sm flex-1">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="relative h-[460px] overflow-hidden rounded-[36px] bg-white/10 shadow-2xl"
            >
              <img src={safeImage} alt={currentItem.name} className="h-full w-full object-cover" />

              <span className="absolute left-4 top-4 rounded-full bg-black/40 px-4 py-1 text-xs font-semibold uppercase tracking-wide backdrop-blur">
          ...
          <button
            type="button"
            className="absolute left-0 top-1/2 hidden -translate-y-1/2 rounded-full border border-white/10 bg-black/50 p-3 text-white transition hover:bg-black/70 md:block"
            onClick={handlePrev}
            aria-label="Anterior"
          >
          ...
          <button
            type="button"
            className="absolute right-0 top-1/2 hidden -translate-y-1/2 rounded-full border border-white/10 bg-black/50 p-3 text-white transition hover:bg-black/70 md:block"
            onClick={handleNext}
        ...
        <div className="flex w-full max-w-md flex-col items-center text-center md:items-start md:text-left">
          <AnimatePresence mode="wait">
            ...
            >
              <div className="inline-flex gap-1 rounded-full bg-white/10 px-4 py-1 text-xs uppercase tracking-[0.3em] text-white/70">
                <span>{currentIndex + 1}</span>
                <span className="text-white/40">/</span>
                <span>{items.length}</span>
              </div>
              <h2 className="mt-4 text-4xl font-black">{currentItem.name}</h2>
              <p className="mt-2 text-lg font-semibold text-[#ffb4a2]">
                {currentItem.price}
                {currentItem.priceGrande ? ` · ${currentItem.priceGrande}` : ''}
              </p>
              <p className="mt-4 text-base text-white/80">{currentItem.description}</p>
              {currentItem.calories ? (
                <span className="mt-4 rounded-full bg-white/10 px-4 py-1 text-xs font-semibold text-white/70">
                  {currentItem.calories}
                </span>
              ) : null}
            </motion.div>
